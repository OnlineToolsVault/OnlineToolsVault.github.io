...
// Long-form copy for /developer-tools/. See CategoryHub.jsx for the inline markup this accepts:
// **bold**, [label](/path/), and [](/path/) for a link labelled from the tool catalogue.
const lede = `{count} tools for the small, constant jobs between writing code and shipping it:
...
this tab, which is what makes it safe to paste a production response into.`

const sections = [
    {
        heading: 'The formatters are one engine with different front doors',
        paragraphs: [
            `[](/html-formatter/), [](/css-formatter/), [](/js-formatter/), [](/json-formatter/),
            ...
            rather than called on a server.`,
            `Use a single-language page when you already know what you have and want it opened,
            ...
            on anyway: two-space indentation and an 80-column print width.`,
            `Understand what a formatter is not. These tools **re-print** your code from a parse
    ...
    {
        heading: 'Formatting as a validity check',
        paragraphs: [
            `Because the printers rebuild a document from a parse tree, they cannot print what they
            ...
            it choked on.`,
            `[](/json-formatter/) is the one to reach for on an unfamiliar payload rather than a
            ...
            four thousand lines looking for the field you were promised.`,
            `[](/sql-formatter/) uppercases keywords and puts one clause per line, and it copes with
    ...
    {
        heading: 'Checking a thing before it runs',
        paragraphs: [
            `[](/regular-expression-tester/) runs your pattern against your own sample text and
            ...
            groups, escapes and lazy quantifiers.`,
            `[](/cron-parser/) turns a crontab expression into a plain-English schedule as you type.
    ...
    {
        heading: 'Colours and data shapes',
        paragraphs: [
            `[](/color-picker/) pairs your system colour picker with one editable hex field and
            re-derives HEX, RGB and HSL from it, so you can copy whichever notation the file you are
            editing wants. It is sRGB only — no CMYK, no alpha channel and no palette management.`,
            `[](/json-to-csv/) flattens JSON into a spreadsheet, turning nested objects into dotted
            ...
            Excel and CSV workbook tools.`
        ]
    },
    {
        heading: 'Why running locally matters here specifically',
        paragraphs: [
            `The paste that goes into a formatter is very often the one you should be most careful
            ...
            never sent anywhere and there is no service to trust with it afterwards.`,
            `The trade-off is honest: the first visit downloads a formatting engine, so a page takes
            ...
            set under [security tools](/security-tools/), which follows the same rule.`
        ]
    }
]

const DeveloperToolsHub = () => <CategoryHub category="developer" lede={lede} sections={sections} />

export default DeveloperToolsHub
