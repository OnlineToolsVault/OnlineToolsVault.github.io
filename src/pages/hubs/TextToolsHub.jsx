...
// Long-form copy for /text-tools/. See CategoryHub.jsx for the inline markup this accepts:
// **bold**, [label](/path/), and [](/path/) for a link labelled from the tool catalogue.
const lede = `{count} tools for the plain-text end of the working day: counting it, comparing two
...
done in this tab, so nothing you paste is transmitted anywhere.`

const sections = [
    {
        heading: 'Counting, and why counters disagree',
        paragraphs: [
            `[](/word-counter/) updates five figures on every keystroke — words, characters,
            sentences, paragraphs and reading time — so you can trim toward a limit and watch the
            number move rather than pressing an Analyse button and waiting.`,
            `It states its rules instead of hiding them, which matters the moment an editor's count
            ...
            else's tool reports a different number.`
        ]
    },
    {
        heading: 'Comparing two versions',
        paragraphs: [
            `[](/diff-viewer/) takes two blocks of text — drafts, config files, log excerpts, source
            ...
            changed.`,
            `It is the right tool whenever a document arrives back with "a few small edits" and you
    ...
    {
        heading: 'Rich text in, Markdown out, and back again',
        paragraphs: [
            `These three tools form a pipeline, and each is useful on its own.`,
            `[](/paste-to-markdown/) takes whatever is on your clipboard from Google Docs, Word,
            ...
            fastest route out of a word processor and into anything that stores plain text.`,
            `[](/markdown-previewer/) renders Markdown live beside the editor as you type, so you
            ...
            with page breaks and selectable text there is [](/markdown-to-pdf/).`,
            `[](/humanize-text/) handles the punctuation that survives a copy and paste and then
            ...
            a character cleanup and nothing more; it does not rewrite your sentences.`
        ]
    },
    {
        heading: 'Filling a layout before the copy exists',
        paragraphs: [
            `[](/lorem-ipsum-generator/) produces 1 to 100 paragraphs, sentences or words of the
            ...
            start editing the words instead of judging the spacing.`,
            `Pair it with [](/word-counter/) when a component has a hard limit: generate to a length,
            count it, and you know what the real copy has to fit into before anyone writes a word of
            it.`
        ]
    },
    {
        heading: 'What these tools deliberately do not do',
        paragraphs: [
            `None of them upload anything, so none of them can offer collaboration, saved documents
            ...
            server.`,
            `For text that has to become a formatted document, cross over to the PDF family:
            [](/create-pdf/) paginates plain text into a PDF with real selectable text, and
            [](/html-to-pdf/) does the same for markup you already have.`
        ]
    }
]

const TextToolsHub = () => <CategoryHub category="text" lede={lede} sections={sections} />

export default TextToolsHub
