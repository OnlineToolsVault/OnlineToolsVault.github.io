...
// Long-form copy for /pdf-tools/. See CategoryHub.jsx for the inline markup this accepts:
// **bold**, [label](/path/), `code`, and [](/path/) for a link labelled from the tool catalogue.
const lede = `{count} tools for working on PDF files, all of them running inside this browser tab.
...
contract or a payslip is safe to open here at all.`

const sections = [
    {
        heading: 'What every tool on this page has in common',
        paragraphs: [
            `Each page loads a PDF library compiled to run in the browser, hands it the file you
            ...
            network panel while a job runs and you will see it stay quiet.`,
            `That sets the limits too: everything uses your own processor and memory, so the jobs
            that render every page to an image are the ones most likely to strain a phone.`
        ]
    },
    {
        heading: 'Structural edits versus redrawn pages',
        paragraphs: [
            `Before choosing a tool, know which of two families it belongs to — that decides what
            survives.`,
            `**Structural tools copy page objects across without redrawing them.** [](/merge-pdf/),
            ...
            Rotating is the extreme case: only a flag changes, so the file size barely moves.`,
            `**Rendering tools turn pages into pixels.** [](/pdf-to-jpg/), [](/pdf-to-png/),
            ...
            has to stay selectable — afterwards there is no text left, only a picture of it.`
        ]
    },
    {
        heading: 'Getting content out of a PDF',
        paragraphs: [
            ...
            result.`,
            `The fix is [](/ocr-pdf/), which recognises the printed English on each page and writes
            ...
            grab it, run recognition first.`,
            `Expect a reflow, not a facsimile: [](/pdf-to-word/) gives one paragraph per line
            without the original fonts, images or columns, and [](/pdf-to-excel/) groups fragments
            sharing a baseline into rows, which works on a printed table and not on a free-form page.`
        ]
    },
    {
        heading: 'Making a PDF out of something else',
        paragraphs: [
            `[](/create-pdf/), [](/markdown-to-pdf/), [](/html-to-pdf/), [](/csv-to-pdf/),
            ...
            too.`,
            `[](/jpg-to-pdf/), [](/image-to-pdf/) and [](/scan-to-pdf/) instead place pictures on the
            ...
            file without a scanner.`
        ]
    },
    {
        heading: 'Privacy, locks and provenance',
        paragraphs: [
            `[](/protect-pdf/) encrypts a document with AES-128 and can switch off copying, editing
            ...
            those pixels were never written.`,
            `Metadata is the quieter risk. [](/pdf-privacy-scanner/) is read-only and reports what is
            ...
            [](/pdf-metadata-editor/) then clear or rewrite those fields without touching a page.`,
            `[](/fingerprint-pdf/) embeds a different hidden identifier in each recipient's copy and
            can read it back out of a leaked file. [](/compare-pdf/) shows what changed between two
            versions as a text diff and a pixel overlay, catching the moved logo a diff never would.`
        ]
    },
    {
        heading: 'When a file is too big, or simply broken',
        paragraphs: [
            `[](/compress-pdf/) repacks the file with object streams and drops its metadata, then
            ...
            document from its rendered pages.`
        ]
    }
]

const PdfToolsHub = () => <CategoryHub category="pdf" lede={lede} sections={sections} />
