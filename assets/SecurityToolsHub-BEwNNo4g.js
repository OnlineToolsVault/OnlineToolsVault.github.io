...
const a = `{count} tools for hashing, encrypting, encoding and inspecting \u2014 the everyday
...
transmitted, logged or stored anywhere.`, o = [{ heading: "Encoding is not encryption, and hashing is neither", paragraphs: ["Three families live here, routinely confused at the moment it matters most.", `**Encodings are reversible and provide no secrecy at all.** [](/base64-encoder/),
            ...
            from nothing else.`, `**Hashes are one-way.** [](/hash-generator/) and [](/bcrypt-generator/) turn input into
            a fixed-length digest that cannot be turned back. They answer "is this the same as
            that?", never "what was this?".`, `**Encryption is reversible, but only with the key.** [](/encrypt-text/),
            [](/decrypt-text/) and [](/file-encryption-tool/) protect content with AES and a
            passphrase you choose. Their strength is your passphrase and nothing else.`] }, { heading: "Choosing a hash", paragraphs: ["[](/hash-generator/) computes MD5, SHA-1, SHA-256, SHA-512 and RIPEMD-160 together, over\n            the UTF-8 bytes of your text, so the digests match `sha256sum`, OpenSSL and Python's\n            `hashlib` character for character. Interoperability is the point: a digest you cannot\n            reproduce on the command line proves nothing.", `[](/file-checksum-generator/) does the same for a file rather than a string, computing
            ...
            against a published checksum.`, `[](/bcrypt-generator/) is different in kind. Bcrypt is **deliberately slow and
            ...
            every run; this page generates hashes and cannot verify one.`] }, { heading: "Encrypting text and files", paragraphs: ["[](/encrypt-text/) uses AES-256-CBC inside the OpenSSL `Salted__` envelope, which\n            means the ciphertext it produces \u2014 the block that starts `U2FsdGVkX1` \u2014 can also be\n            decrypted from a command line by anyone with the passphrase, not only here.\n            [](/decrypt-text/) is the other half, for a message you have been sent.", `[](/file-encryption-tool/) applies the same idea to any file, with one caveat stated
            ...
            passphrase and it is sound; use a dictionary word and the algorithm cannot save you.`, `[](/password-strength-checker/) estimates entropy in bits and translates it into how
            ...
            deserves. Treat the offline figure as the realistic one.`] }, { heading: "Reading tokens without trusting them", paragraphs: [`[](/jwt-decoder/) splits a JSON Web Token and shows its header and payload claims \u2014
            issuer, subject, expiry, scopes \u2014 usually all you need when an API returns 401 and you
            want to know whether the token expired or was issued for the wrong audience.`, `Read the limitation twice: the signature is displayed but **never verified**. A decoded
            ...
            tab, which is more than can be said for pasting a live session token into a hosted site.`, `[](/uuid-generator/) produces 1 to 100 random version 4 UUIDs per run in canonical
            lowercase RFC 9562 form, one per line. Version 4 is random rather than sequential, so
            they leak nothing about when they were made \u2014 and do not sort in creation order.`] }, { heading: "Why the local rule matters most in this category", paragraphs: [`Every category on this site runs in the browser, but here it is the whole point. A
            ...
            of those services have to be malicious to hurt you; they only have to keep logs.`, `These pages have nothing to log: the computation happens in code your browser already
            ...
            [developer tools](/developer-tools/) cover neighbouring ground.`] }], h = () => e.jsx(t, { category: "security", lede: a, sections: o });
export {
  h as default
};
