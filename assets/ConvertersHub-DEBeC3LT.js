...
const a = `{count} tools for moving data from the shape it arrived in to the shape you need:
...
finishes in this tab without an upload.`, o = [{ heading: "Spreadsheets, CSV and JSON", paragraphs: [`[](/csv-to-json/) reads a CSV and returns an array of objects keyed by the header row \u2014
            ...
            into dotted column names.`, `[](/csv-to-excel/) produces a real \`.xlsx\` workbook, which matters more than it
            ...
            Excel, Sheets and Numbers.`, "[](/excel-to-csv/) is the return trip and is deliberately narrow: it takes the **first\n            sheet** of an `.xlsx` or `.xls` workbook and writes the **displayed** cell values,\n            so what you get is what the spreadsheet showed rather than the formulas underneath. If\n            the data needs to end up as a printed table instead, [](/csv-to-pdf/) and\n            [](/excel-to-pdf/) typeset it onto real pages with selectable text."] }, { heading: "Audio and video", paragraphs: [`[](/video-to-audio/) pulls the audio track out of an MP4, MOV or AVI and saves it as a
            ...
            channel count rather than quietly resampling.`, `Both run FFmpeg compiled to WebAssembly inside the page, which has a visible cost and a
            ...
            your connection, no server-imposed size cap, and no copy of your footage anywhere.`] }, { heading: "Archives, and looking before you extract", paragraphs: [`[](/zip-file-creator/) builds a ZIP from files on your machine \u2014 add and remove entries,
            name the archive, then pack at deflate level six, a middle setting that compresses well
            without a long wait.`, `[](/zip-viewer/) is the more interesting half. It lists what is inside an archive with
            ...
            "what is actually in this attachment?" before you commit to extracting it.`] }, { heading: "Numbers, times and sizes", paragraphs: [`[](/timestamp-converter/) converts a Unix epoch timestamp into a readable date and back,
            with the current timestamp ticking live and an ISO 8601 form alongside \u2014 enough to settle
            most "is this seconds or milliseconds?" arguments at a glance.`, `[](/unit-converter/) covers four families and says so up front: length, weight,
            temperature and digital storage. There is no area, speed or currency conversion here, so
            you will not waste time hunting for a tab that is not there.`, `[](/file-size-calculator/) takes one figure and shows it simultaneously in bytes, KB,
            MB, GB, TB and bits, on 1024-based units. It is a pure converter \u2014 there is no
            transfer-time estimate, because that would need a link speed it has no way to know.`] }, { heading: "Working with the files themselves", paragraphs: [`[](/batch-file-renamer/) applies a find-and-replace, a prefix, a suffix and a counter
            ...
            safety net in practice.`, `[](/file-metadata-viewer/) reports a file's name, MIME type, size, extension and last
            ...
            [](/pdf-privacy-scanner/) for documents.`] }, { heading: "Producing something new", paragraphs: [`Three tools here generate a document rather than convert one. [](/qr-generator/) turns
            a URL or any text into a QR code in your own colours and downloads it as a PNG; Wi-Fi and
            vCard codes work if you type the standard string yourself, as there is no wizard.`, `[](/gst-invoice-generator/) produces an Indian GST tax invoice with the CGST/SGST or
            ...
            your own device.`] }], d = () => e.jsx(t, { category: "utility", lede: a, sections: o });
export {
  d as default
};
