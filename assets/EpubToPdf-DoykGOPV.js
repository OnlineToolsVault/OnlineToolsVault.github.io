import { r as P, j as e, L as De } from "./index-OUpguYFg.js";
import { R as Ve } from "./RelatedTools-dQ1AUZ0r.js";
import { T as _e } from "./ToolLayout-CuKFTkh4.js";
import { F as Ze } from "./FileUploader-Dhw1e8vw.js";
import { J as Xe } from "./jszip.min-BycgvNgQ.js";
import { P as Ye, S as pe, r as ne } from "./PDFButton-DYmqjJK7.js";
import { F as Je } from "./FileSaver.min-DzHDzKVl.js";
import { m as ge } from "./tools-B3OPepIK.js";
import { A as Qe } from "./alert-triangle-BqnKTzYa.js";
import { D as et } from "./download-DqlBxbZM.js";
import { A as tt } from "./align-left-C-CKuseV.js";
import { S as nt } from "./shield-check-DCjAjSWE.js";
import "./index-CBYUSgtG.js";
import "./_commonjs-dynamic-modules-TDtrdbi3.js";
import "./UPNG-CjUEgNm-.js";
import "./shield-BrCBnKXk.js";
const rt = /* @__PURE__ */ new Set(["\u20AC", "\u201A", "\u0192", "\u201E", "\u2026", "\u2020", "\u2021", "\u02C6", "\u2030", "\u0160", "\u2039", "\u0152", "\u017D", "\u2018", "\u2019", "\u201C", "\u201D", "\u2022", "\u2013", "\u2014", "\u02DC", "\u2122", "\u0161", "\u203A", "\u0153", "\u017E", "\u0178"]), ot = /* @__PURE__ */ new Set([173, 8203, 8204, 8205, 8288, 65279]), at = /[\u00ad\u200b-\u200d\u2060\ufeff]/g, fe = (r) => String(r ?? "").replace(at, ""), ue = { a4: { label: "A4 (210 \xD7 297 mm)", width: 595.28, height: 841.89 }, letter: { label: "Letter (8.5 \xD7 11 in)", width: 612, height: 792 }, a5: { label: "A5 (148 \xD7 210 mm)", width: 419.53, height: 595.28 } }, st = /* @__PURE__ */ new Set(["application/xhtml+xml", "text/html", "application/xml", "text/xml", "application/x-dtbook+xml", "application/xhtml-xml", "text/x-oeb1-document"]), it = /\.(x?html?|xml|xht)$/i, lt = /* @__PURE__ */ new Set(["mobi", "azw", "azw1", "azw3", "azw4", "kfx", "prc", "tpz", "pdb"]), ct = /* @__PURE__ */ new Set(["pdf", "doc", "docx", "odt", "rtf", "txt", "md", "fb2", "djvu", "lit", "cbz", "cbr", "chm", "jpg", "jpeg", "png", "gif", "webp", "mp3", "mp4"]), be = (r) => fe(r).replace(/\s+/g, " ").trim(), me = (r) => fe(r).replace(/\r\n?/g, `
`).replace(/[^\S\n]+/g, " ").split(`
`).map((n) => n.trim()).join(`
`).replace(/\n{3,}/g, `
...
`).replace(/\t/g, "    ").replace(/[^\S\n]+$/gm, "").replace(/^\n+|\n+$/g, ""), Ne = (r) => {
  ...
  const a = [];
  for (const t of `${o.startsWith("/") ? "" : r}${o}`.split("/")) t === "" || t === "." || (t === ".." ? a.pop() : a.push(t));
  return a.join("/");
}, oe = async (r) => {
  const n = await r.async("uint8array");
  if (n.length >= 2) {
  ...
  if (n) {
    const a = o.parseFromString(r, "application/xml");
    if (a.getElementsByTagName("parsererror").length === 0) return a;
  }
  return o.parseFromString(r, "text/html");
}, M = (r) => String(r.tagName || "").replace(/^.*:/, "").toLowerCase(), O = (r, n) => {
  const o = r.getElementsByTagName(n);
  if (o.length > 0) return Array.from(o);
  const a = n.replace(/^.*:/, "").toLowerCase();
  return Array.from(r.getElementsByTagName("*")).filter((t) => M(t) === a);
}, H = (r) => Object.assign(new Error(r), { readable: true }), ht = async (r) => {
  const n = r.file("META-INF/encryption.xml"), o = /* @__PURE__ */ new Set();
  if (!n) return o;
  let a;
  try {
    a = ae(await oe(n), true);
  } catch {
    return o;
  }
  ...
  for (const B of O(i, "itemref")) {
    ...
    if (x.properties.split(/\s+/).includes("nav") || f.has(x.path)) {
      s.nav += 1, r.file(x.path) && m.push(x);
      continue;
    }
    if (!r.file(x.path)) {
      s.missing += 1;
...
`;
  ...
  return t(r), { text: o, images: a };
}, ft = (r) => {
  const n = r.parentNode;
  if (!n || n.nodeType !== 1 || M(n) !== "ol") return "\u2022";
  const o = parseInt(n.getAttribute("start"), 10);
  ...
  for (const t of Array.from(n.childNodes)) {
    ...
    if (Number.isFinite(h) && (a = h), t === r) return `${a}.`;
    a += 1;
  }
  return "\u2022";
}, bt = (r) => {
...
`;
        ...
        if (Ce.has(c)) {
          t();
          const f = c === "LI" ? Array.from(l.children).filter((m) => gt.has(M(m).toUpperCase())) : [], S = f.length > 0 ? re(l, (m) => f.includes(m)) : re(l);
          if (o += S.images, c === "PRE") {
            const m = dt(S.text);
    ...
    t();
  }, u = r.getElementsByTagName("body")[0] || r.documentElement;
  return u && h(u, 0), { blocks: n, images: o };
}, yt = (r) => Array.from(r.getElementsByTagName("nav")).some((n) => `${n.getAttribute("epub:type") || n.getAttribute("type") || ""} ${n.getAttribute("role") || ""}`.split(/\s+/).some((a) => a === "toc" || a === "doc-toc")), xt = (r, n, o, a) => {
  if (yt(r)) return true;
  const t = /* @__PURE__ */ new Set();
  let h = 0;
  for (const i of Array.from(r.getElementsByTagName("a"))) {
    const p = i.getAttribute("href");
    if (!p || p.startsWith("#") || /^[a-z][a-z0-9+.-]*:/i.test(p)) continue;
    const l = J(a, p);
    o.has(l) && (t.add(l), h += be(i.textContent).length);
  }
  ...
  if (!r) return false;
  const n = r.slice(0, 4096);
  if (n.trim() && !n.includes("<")) return true;
  let o = 0;
  for (let a = 0; a < n.length; a += 1) {
    const t = n.charCodeAt(a);
    t === 9 || t === 10 || t === 13 || (t < 32 || t === 65533) && (o += 1);
  }
  return o / n.length > 0.02;
}, kt = (r, n, o, a) => {
  const t = [], h = String(r).split(/\s+/).filter(Boolean);
  if (h.length === 0) return t;
  const u = (l) => n.widthOfTextAtSize(l, o), i = (l) => {
    let c = "";
  ...
  const [r, n] = P.useState(null), [o, a] = P.useState(null), [t, h] = P.useState(null), [u, i] = P.useState(null), [p, l] = P.useState(false), [c, f] = P.useState(false), [S, m] = P.useState(0), [s, N] = P.useState(null), [$, se] = P.useState("a4"), [W, B] = P.useState(11), [j, x] = P.useState(true), [V, Ue] = P.useState(true), [ie, ze] = P.useState(true), _ = P.useRef(false), Q = P.useRef(0), Oe = () => {
    ...
    const Z = (E) => {
      ...
      for (const U of String(E ?? "")) {
        ...
        ot.has(D) || (D === 10 ? I += `
` : D === 9 || D === 13 ? I += " " : D >= 32 && D <= 126 ? I += U : D === 160 ? I += " " : D > 160 && D <= 255 || rt.has(U) ? I += U : (I += "?", ee += 1));
      }
      return I;
    };
    try {
      ...
      for (let b = 0; b < t.spine.length; b += 1) {
        ...
        const k = t.spine[b], y = await oe(o.file(k.path));
        if (wt(y)) {
          le += 1;
          continue;
        }
...
`)) {
      ...
      V && (te(), v = q.height * 0.62, G(t.title || r.name.replace(/\.epub$/i, ""), ce, Math.min(30, T + 15), 0, de), v -= T, t.creator && G(t.creator, X, T + 3, 0, ne(0.35, 0.4, 0.47))), E.forEach((b, k) => {
        const y = T + 7;
      ...
      }), F.setTitle(t.title || r.name.replace(/\.epub$/i, "")), t.creator && F.setAuthor(t.creator), F.setLanguage(t.language || "en"), F.setProducer("OnlineToolsVault eBook to PDF"), F.setCreator("OnlineToolsVault eBook to PDF");
      const Ge = await F.save();
      if (w()) return;
      m(100);
      const Ie = new Blob([Ge], { type: "application/pdf" });
      Je.saveAs(Ie, `${r.name.replace(/\.epub$/i, "") || "book"}.pdf`), N({ chapters: E.length, pages: Be.length, skippedImages: D, replaced: ee, navSkipped: ve, unreadable: le, nonText: ((_c = t.ignored) == null ? void 0 : _c.nonText) || 0, missing: ((_d = t.ignored) == null ? void 0 : _d.missing) || 0, size: Ie.size });
    } catch (E) {
