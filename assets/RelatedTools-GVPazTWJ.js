import { u as v, r as M, j as s, L as R } from "./index-BtmU1OS0.js";
import { t as y } from "./tools-DOXC7sEs.js";
...
const l = b.map((e) => ({ tool: e, index: g.get(e.path), size: i.get(e.category).length })).sort((e, t) => {
  const r = (2 * e.index + 1) * t.size, n = (2 * t.index + 1) * e.size;
  return r !== n ? r - n : e.tool.path < t.tool.path ? -1 : 1;
  ...
  const t = w(e), r = z.get(t), n = [], f = /* @__PURE__ */ new Set([t]), x = (o) => !o || f.has(o.path) ? false : (f.add(o.path), n.push(o), true);
  if (r) {
    const o = i.get(r.category) || [], h = g.get(t) ?? 0;
    for (let a = 1; a < o.length && n.length < k; a += 1) x(o[(h + a) % o.length]);
  }
  const c = T - n.length;
  if (c > 0 && l.length > 0) {
    const o = r ? N.get(t) : S(t), h = E(c);
    for (let a = 0; a < c; a += 1) {
      const j = (o + h[a]) % l.length;
      let m = false;
